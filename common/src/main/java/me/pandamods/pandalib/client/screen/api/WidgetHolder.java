...
public interface WidgetHolder {
	List<WidgetImpl> widgets();

	default void onMouseMove(double mouseX, double mouseY) {
		widgets().stream().filter(WidgetImpl::isHovered).forEach(widget -> widget.mouseMoved(mouseX, mouseY));
	}

	default boolean onMouseClick(double mouseX, double mouseY, int button) {
		Optional<WidgetImpl> widgetOptional = widgets().stream().filter(WidgetImpl::isHovered).findFirst();
