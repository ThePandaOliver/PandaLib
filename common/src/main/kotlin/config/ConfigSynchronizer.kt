	...
	internal fun init() {
		...
		if (configs.isNotEmpty()) {
			serverPlayerJoinEvent += { player ->
				...
				pandalibLogger.debug("Sending all server configs to {}", player.name)
				...
				pandalibLogger.debug("Sending previous client configs to {}", player.name)
				createConfigPayloadsWithClientConfigs()
					.takeIf { it.isNotEmpty() }
					?.let { ServerPlayNetworking.send(player, it) }

				// Send request for all client's configs
				pandalibLogger.debug("Sending config request to {}", player.name)
