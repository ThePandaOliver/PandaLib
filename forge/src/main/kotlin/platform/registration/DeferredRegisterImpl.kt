...
class DeferredRegisterImpl : DeferredRegisterPlatform {
	...
	fun registerNewRegistries() {
		if (BuiltInRegistries.REGISTRY is MappedRegistry<*>)
				(BuiltInRegistries.REGISTRY as MappedRegistry<*>).unfreeze()

		for (registry in pendingRegistryTypes) {
			(BuiltInRegistries.REGISTRY as WritableRegistry<Registry<*>>)
				.register(registry.key() as ResourceKey<Registry<*>>, registry, registry.registryLifecycle())
		}

		if (BuiltInRegistries.REGISTRY is MappedRegistry<*>)
				(BuiltInRegistries.REGISTRY as MappedRegistry<*>).freeze()
	}
