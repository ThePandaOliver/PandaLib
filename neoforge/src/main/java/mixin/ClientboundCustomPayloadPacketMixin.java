...
public class ClientboundCustomPayloadPacketMixin {
	...
	private static StreamCodec<FriendlyByteBuf, CustomPacketPayload> wrapPlayCodec(
			CustomPacketPayload.FallbackProvider<FriendlyByteBuf> fallbackProvider, List<CustomPacketPayload.TypeAndCodec<? super FriendlyByteBuf, ?>> list,
		...
		StreamCodec<FriendlyByteBuf, CustomPacketPayload> codec = original.call(fallbackProvider, list, connectionProtocol, packetFlow);
		PandaLibCustomPayloadPacketCodec<FriendlyByteBuf> pandalibCodec = (PandaLibCustomPayloadPacketCodec<FriendlyByteBuf>) codec;
		pandalibCodec.pandalibSetPacketCodecProvider((byteBuf, resourceLocation) -> PacketRegistry.CLIENT_PACKET_CODEC.get(resourceLocation));
		return codec;
	}
